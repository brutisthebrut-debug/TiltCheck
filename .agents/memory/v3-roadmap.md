...
**How to apply:** Before planning or accepting any new feature work, open `.local/tasks/v3-roadmap-log.md`. Wave A (ship blockers) is COMPLETE as of 2026-07-16 — cross-crew read privacy now covers bets, parlays, AND /stats/recent-activity. Wave B code-side items (#21 delete warning, #184 AI fallback, #185 standings proof, #164 badge legibility) are also COMPLETE as of 2026-07-16; the only remaining V3 items are post-publish live-site verifications (#160, #88/#89) — V3 cannot be closed without a publish. Keep the log updated (move items to Done with date + one-liner) exactly like the v2 log. Durable lesson: any endpoint returning another bettor's plays must use the crew-aware social scope, not just world scoping — an audit that checks only the obvious list endpoints will miss feeds like recent-activity.
