...
**How to apply:** Before planning or accepting any new feature work, open `.local/tasks/v3-roadmap-log.md`. Wave A (ship blockers) is COMPLETE as of 2026-07-16 — cross-crew read privacy now covers bets, parlays, AND /stats/recent-activity. Wave B (verify & harden) items remain; steer there before any V4 work. Keep the log updated (move items to Done with date + one-liner) exactly like the v2 log. Durable lesson: any endpoint returning another bettor's plays must use the crew-aware social scope, not just world scoping — an audit that checks only the obvious list endpoints will miss feeds like recent-activity.
