...
**How to apply:** any future LLM feature: (1) lazy-import the client, (2) cache results in the DB with a unique index per natural key, (3) singleflight concurrent misses via an in-process promise map — chain `.catch(() => {}).finally(cleanup)` on the side-channel or you get unhandled rejections, (4) bound spend on the public demo mount (demo only generates for the latest completed week) AND for member history browsing (fresh generations for non-latest weeks draw from a small per-user daily budget counted from rows created today; cached weeks and the latest week are exempt — the route returns `limitReached: true` so the UI can explain instead of hiding), (5) prompt receives a compact computed-facts JSON only — never raw DB rows — with a facts-only/no-picks system prompt.
