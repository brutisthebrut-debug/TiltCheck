...
/**
 * Confirms the API can reach PostgreSQL before a deployment receives traffic.
...
/**
 * Creates a crew owned by the signed-in user and switches them to it. The first crew is free; creating another while already in one is part of paid multi-Crew access (402 `pro_required` for free accounts — founder and live subscriptions pass).
...
/**
 * Joins the crew behind the invite code and switches to it. Joining your first Crew is free; joining another while already in one requires paid multi-Crew access (402 `pro_required` for free accounts).
...
/**
 * Owner only. Creates a challenge that runs from today through the next 7 days. Only one challenge can be active at a time per crew (409 `challenge_active` when one is already running). Every Crew can run challenges and review the results.
...
/**
 * @summary Recent bets and parlays activity feed across the active Crew
 */
...
/**
 * Reads the bettor's settled history in the specified sport and bet type, then generates a 2–3 sentence coaching note via AI. Ephemeral — not persisted. Rate-limited to 10 checks per hour per bettor. Demo sessions return a deterministic simulated response.
 * @summary Get Arc's pre-bet coaching note for an in-progress bet
 */
...
/**
 * Returns the signed-in bettor's key metrics (ROI, win rate, calibration, post-mortem rate, tilt frequency) compared against pre-computed platform-wide percentile breakpoints. Percentiles are computed weekly across all opted-in, non-demo users with ≥10 settled plays. The bettor's percentile band is computed server-side — raw per-user data is never exposed. Returns `opted_out: true` when the user has disabled benchmarking on their account.
 * @summary Get the bettor's metrics ranked against anonymized platform-wide percentiles
 */
...
/**
 * @summary Get active Crew info and members
 */
...
/**
 * Head-to-head comparison rows for every Crew member. Money math includes settled straight bets AND parlays (won/lost/push), excludes dead-zone-odds rows, and honors the same settledAt window as the leaderboard: week = last 7 days, month = last 30 days, all = everything.
 * @summary Compare bets and stats between Crew members
 */
...
/**
 * Server-verified subscription status. When the cached horizon has expired, the server re-checks the payment provider and stores the new horizon — paid access is never granted from client state, redirects, or checkout IDs. Founders bypass the paid Crew limit; the demo board has no checkout flow.
 * @summary Paid multi-Crew status for the signed-in bettor
 */
 ...
 * @summary Create a hosted checkout session for additional Crew access
 */
